...
const otherHymns: HymnType[] = Object.entries(hymns['Other Hymns'] || {}).map(([title, content]) => ({
...
export default function OtherHymnsScreen() {
  ...
  const [selectedHymnId, setSelectedHymnId] = useState<string>(
    typeof paramHymn === 'string' ? paramHymn : otherHymns[0]?.id || ''
  );

  const selectedHymn = otherHymns.find(h => h.id === selectedHymnId);
