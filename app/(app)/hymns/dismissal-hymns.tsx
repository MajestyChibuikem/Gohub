...
const dismissalHymns: HymnType[] = Object.entries(hymns['Dismissal'] || {}).map(([title, content]) => ({
...
export default function DismissalHymnsScreen() {
  ...
  const [selectedHymnId, setSelectedHymnId] = useState<string>(
    typeof paramHymn === 'string' ? paramHymn : dismissalHymns[0]?.id || ''
  );

  const selectedHymn = dismissalHymns.find(h => h.id === selectedHymnId);
