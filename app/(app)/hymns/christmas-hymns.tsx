...
// Get Christmas hymns from the hymnIndex
const christmasHymns: HymnType[] = Object.entries(hymns['Christmas'] || {}).map(([title, content]) => ({
...
export default function ChristmasHymnsScreen() {
  const { hymn: paramHymn } = useLocalSearchParams();
  const { theme, getFontSize } = useTheme();
  const { language } = useLanguage();
  const styles = createHymnStyles(theme);

  console.log('📱 christmas-hymns.tsx - Component loaded');
  console.log('🎵 Hymn param received:', paramHymn);

  const [selectedHymnId, setSelectedHymnId] = useState<string>(
    typeof paramHymn === 'string' ? paramHymn : christmasHymns[0]?.id || ''
  );

  console.log('🎯 Selected hymn ID:', selectedHymnId);
  console.log('📚 Available hymns:', christmasHymns.map(h => h.id));

  const selectedHymn = christmasHymns.find(h => h.id === selectedHymnId);
