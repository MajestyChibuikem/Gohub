...
const offertoryHymns: HymnType[] = Object.entries(hymns['Offertory'] || {}).map(([title, content]) => ({
...
export default function OffertoryHymnsScreen() {
  ...
  const [selectedHymnId, setSelectedHymnId] = useState<string>(
    typeof paramHymn === 'string' ? paramHymn : offertoryHymns[0]?.id || ''
  );

  const selectedHymn = offertoryHymns.find(h => h.id === selectedHymnId);
