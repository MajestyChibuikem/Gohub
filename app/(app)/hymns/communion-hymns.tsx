...
// Get Communion hymns from the hymnIndex
const communionHymns: HymnType[] = Object.entries(hymns['Communion'] || {}).map(([title, content]) => ({
...
export default function CommunionHymnsScreen() {
  const { hymn: paramHymn } = useLocalSearchParams();
  const { theme, getFontSize } = useTheme();
  const { language } = useLanguage();
  const styles = createHymnStyles(theme);

  console.log('📱 communion-hymns.tsx - Component loaded');
  console.log('🎵 Hymn param received:', paramHymn);

  const [selectedHymnId, setSelectedHymnId] = useState<string>(
    typeof paramHymn === 'string' ? paramHymn : communionHymns[0]?.id || ''
  );

  console.log('🎯 Selected hymn ID:', selectedHymnId);
  console.log('📚 Available hymns:', communionHymns.map(h => h.id));

  const selectedHymn = communionHymns.find(h => h.id === selectedHymnId);
